  ...
  public getWalletPublicKey(customProvider?: string): PublicKey | null {
    ...
    try {
      ...
      if (provider === "jupiter") {
        ...
        if (process.env.TEST_MODE === "true" || process.env.SIMULATION_MODE === "true") {
          console.log("🧪 TEST MODE - Creating dummy wallet for simulation");
          
          // Create a dummy keypair for testing
          const { Keypair } = require('@solana/web3.js');
          const dummyKeypair = Keypair.generate();
          
          // Return just the public key (matching the expected return type)
          return dummyKeypair.publicKey;
        }
