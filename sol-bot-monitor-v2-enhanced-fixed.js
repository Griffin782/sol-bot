// SOL-BOT V2 Advanced Monitor - ENHANCED VERSION (FIXED)
...
const CONFIG = {
  ...
  specificLogFile: null // Will be set if a specific file is provided
};

// Check if argument is a file path instead of a directory
if (CONFIG.logDir && CONFIG.logDir.endsWith('.log') && fs.existsSync(CONFIG.logDir)) {
  CONFIG.specificLogFile = CONFIG.logDir;
  CONFIG.logDir = path.dirname(CONFIG.logDir);
  console.log(`Analyzing specific log file: ${CONFIG.specificLogFile}`);
}
...
const PATTERNS = {
  ...
  // Enhanced metadata error patterns
  ...
  // Safety and quality filter patterns
...
// Pattern matching to detect specific issues
...
function getLatestLogFile() {
  try {
    // If a specific log file is provided, use that
    if (CONFIG.specificLogFile && fs.existsSync(CONFIG.specificLogFile)) {
      return CONFIG.specificLogFile;
    }
    
    // Otherwise find the most recent log file
    const files = fs.readdirSync(CONFIG.logDir)
      .filter(file => file.endsWith('.log') && file.startsWith('bot-run-')) // Filter to just bot log files
      .map(file => ({
        name: file,
    ...
    // Only get logs from today and yesterday
    const now = new Date();
    ...
    return recentFiles.length > 0 ? recentFiles[0].path : (files.length > 0 ? files[0].path : null);
  } catch (err) {
...
async function processLogFile(filePath, lastPosition = 0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      // Check for safety and quality filter events
...
function analyzeResults() {
  ...
  // Check for quality filter issues
  ...
  // Check for safety system issues
...
function displayHeader(runtime) {
  ...
  if (CONFIG.specificLogFile) {
    console.log(`${colors.fg.cyan}Analyzing: ${path.basename(CONFIG.specificLogFile)} | Last Update: ${new Date().toLocaleTimeString()}${colors.reset}\n`);
  } else {
    ...
    try {
      ...
      if (!logFile) {
        ...
        if (!logFile) {
          ...
          console.log(`Found log file: ${path.basename(logFile)}`);
        }
      }
      ...
      // Auto-exit if test is complete or we're using a specific file
      if (CONFIG.specificLogFile || analysis.runtime >= CONFIG.testDuration) {
        if (CONFIG.specificLogFile) {
          console.log(`\n${colors.bg.green}${colors.fg.black} ANALYSIS COMPLETE - MONITOR EXITING ${colors.reset}`);
        } else {
        ...
        // Exit if we're analyzing a specific file
        if (CONFIG.specificLogFile) {
          process.exit(0);
        }
      }
      ...
      // If we're analyzing a specific file, only do one pass
      if (CONFIG.specificLogFile) {
        return;
      }
