// ⚠️ DEPRECATED: This file is ABANDONED - DO NOT USE!
// ⚠️ USE z-new-controls/z-masterConfig.ts instead
// ⚠️ This file exists only for reference - importing will cause errors
...
console.error("🚨 File: src/enhanced/masterConfig.ts is ABANDONED");
throw new Error("DEPRECATED CONFIG FILE - Use z-new-controls/z-masterConfig.ts");

// masterConfig.ts - Complete Trading Bot Configuration [DEPRECATED]
// Version: Test 8.7 Enhanced [ABANDONED]
...
const config: MasterConfig = {
  ...
  testMode: false,               // Set to false for live trading
  verboseLogging: true,
  ...
  // Runtime Controls
  runtime: {
    duration: 0,              // 5m =300 1h =3600 6h =21600 0 = run indefinitely
    maxRuntime: 86400,             // 24 hours max
    pauseBetweenScans: 100,      // 1 second between scans
    reconnectAttempts: 5,
    reconnectDelay: 5000,
  ...
  pool: {
    initialPool: 600,
    targetPool: 100000,
    minPoolReserve: 10,             // Reduced to 5% for aggressive mode
    maxPoolRisk: 15,               // Increased to 80% for full utilization
    compoundProfits: true,
    
    positionSize: 0.089,           // ~$40 at current SOL price
    dynamicSizing: true,           // Enable dynamic sizing
    minPositionSize: 0.05,
    maxPositionSize: 0.5,
    positionSizePercent: 3,        // 5% of pool per position
