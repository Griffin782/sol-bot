...
// ============================================
console.log("=== LOADING CONFIGURATION FROM masterConfig.ts ===");
console.log(`Initial Pool: $${masterConfig.pool.initialPool}`);
console.log(`Duration: ${masterConfig.runtime.duration === 0 ? 'Unlimited' : masterConfig.runtime.duration + ' seconds'}`);
console.log(`Max Positions: ${masterConfig.pool.maxPositions}`);
console.log(`Position Size: ${masterConfig.pool.positionSize} SOL`);
console.log("==================================================\n");

// Map masterConfig values to local config
const config = {
  // Network settings
  rpcUrl: masterConfig.network.rpcEndpoints[0] || 'https://api.mainnet-beta.solana.com',
  wsUrl: masterConfig.network.wsEndpoints[0] || 'wss://api.mainnet-beta.solana.com',
  
  // Program ID (this stays hardcoded as it's specific to pump.fun)
  pumpFunProgramId: '6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P',
  
  // Runtime settings from masterConfig
  testDurationMs: masterConfig.runtime.duration * 1000, // Convert seconds to milliseconds
  pauseBetweenScans: masterConfig.runtime.pauseBetweenScans,
  checkInterval: masterConfig.runtime.checkInterval,
  heartbeatInterval: masterConfig.runtime.heartbeatInterval,
  
  // Pool settings from masterConfig
  ...
  minPoolReserve: masterConfig.pool.minPoolReserve,
  maxPoolRisk: masterConfig.pool.maxPoolRisk,
  
  // Entry criteria from masterConfig
  minLiquidity: masterConfig.entry.minLiquidity,
  maxLiquidity: masterConfig.entry.maxLiquidity,
  minMarketCap: masterConfig.entry.minMarketCap,
  maxMarketCap: masterConfig.entry.maxMarketCap,
  minHolders: masterConfig.entry.minHolders,
  maxHolders: masterConfig.entry.maxHolders,
  minWhaleCount: masterConfig.entry.minWhaleCount,
  minWhaleBuyVolume: masterConfig.entry.minWhaleBuyVolume,
  honeypotCheck: masterConfig.entry.honeypotCheck,
  rugCheck: masterConfig.entry.rugCheck,
  maxSlippage: masterConfig.entry.maxSlippage,
  
  // Exit strategy from masterConfig
  stopLoss: masterConfig.exit.stopLoss,
  takeProfit1: masterConfig.exit.takeProfit1,
  takeProfit2: masterConfig.exit.takeProfit2,
  takeProfit3: masterConfig.exit.takeProfit3,
  maxHoldTime: masterConfig.exit.maxHoldTime,
  minHoldTime: masterConfig.exit.minHoldTime,
  whaleSellPercentage: masterConfig.exit.whaleSellPercentage,
  whaleSellVolume: masterConfig.exit.whaleSellVolume,
  
  // Whale tracking from masterConfig
  whaleTrackingEnabled: masterConfig.whales.enabled,
  minWhaleBalance: masterConfig.whales.minWhaleBalance,
  whaleTrackingInterval: masterConfig.whales.trackingInterval,
  
  // Execution settings from masterConfig
  priorityFee: masterConfig.execution.priorityFee,
  slippageTolerance: masterConfig.execution.slippageTolerance,
  executionSpeed: masterConfig.execution.executionSpeed,
  maxRetries: masterConfig.execution.maxRetries,
  
  // Performance settings from masterConfig
  targetWinRate: masterConfig.performance.targetWinRate,
  maxDailyLoss: masterConfig.performance.maxDailyLoss,
  maxConsecutiveLosses: masterConfig.performance.maxConsecutiveLosses,
  
  // Reporting settings from masterConfig
  generateReports: masterConfig.reporting.generateReports,
  reportInterval: masterConfig.reporting.reportInterval * 60000, // Convert minutes to milliseconds
  verboseLogging: masterConfig.verboseLogging,
...
async function runPoolIntegratedDetection() {
  ...
  console.log(`=====================================`);
  console.log(`📋 Version: ${masterConfig.botVersion}`);
...
function printFinalPoolReport(queueManager: TokenQueueManager): void {
  ...
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🏆 FINAL REPORT - ${masterConfig.botVersion}`);
...
console.log(`💡 STARTING ${masterConfig.botVersion}`);
console.log(`===========================================`);
console.log(`🏦 Pool-integrated token detection system`);
console.log(`🔄 Using configuration from masterConfig.ts`);
