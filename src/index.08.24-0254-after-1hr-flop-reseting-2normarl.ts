...
const POOL_SESSIONS: PoolSession[] = [
  { 
    sessionNumber: 1, 
    initialPool: 600, 
    netTarget: 1000,      // You keep this
    grossTarget: 1400,   // Must reach this
    reinvestmentPercent: 0.70,
    nextSessionPool: 700 
  },
  { 
    sessionNumber: 2, 
    initialPool: 700, 
    netTarget: 2000,    // You keep this
    grossTarget: 2800,   // Must reach this
    reinvestmentPercent: 0.50,
    nextSessionPool: 1000 
  },
  { 
    sessionNumber: 3, 
    initialPool: 1000, 
  ...
  { 
    ...
    netTarget: 2000,   // You keep this
    grossTarget: 2800,  // Must reach this
    reinvestmentPercent: 0.50,  // Only 20% for session 4+
    nextSessionPool: 1000      // Stays at 40k
  },
];
...
if (queueManager && queueManager.hasTokenInQueue?.(returnedMint)) {
  ...
  if (result) {
    ...
    if (queueManager) {
      try {
        const signature = ""; // Get from actual transaction
        await queueManager.onNewTokenDetected(signature, returnedMint);
      } catch (error) {
