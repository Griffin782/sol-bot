...
export class MarketRecorder extends EventEmitter {
  ...
  private tokensBeingTracked: Set<string> = new Set(); // Race condition fix
  ...
  async initialize(): Promise<void> {
    ...
    const dbPath = isBaselineMode
      ? getCurrentDatabasePath('baseline')
      : getCurrentDatabasePath('bot');
  ...
  private async getCurrentPrice(mint: string): Promise<number | null> {
    try {
      ...
      const bondingCurvePubkey = new PublicKey(bondingCurveInfo.poolAddress);
   ...
   */
  private parseBondingCurveData(accountData: Buffer): { virtualTokenReserves: bigint; virtualSolReserves: bigint } | null {
    try {
   ...
   */
  private calculatePriceFromReserves(data: { virtualTokenReserves: bigint; virtualSolReserves: bigint }): number | null {
    try {
